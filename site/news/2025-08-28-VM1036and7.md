...
All the fixes are intended to improve the experience when having applications that have a lot of open idle sockets; so if it is your case, you can profit from the new release (Again, only in Windows).
...
This release has only a fix for Windows Sockets. It is a problem we have found with @SabineM after a lot of debugging. The problem was heavy CPU usage when there were a lot of idle sockets. 
